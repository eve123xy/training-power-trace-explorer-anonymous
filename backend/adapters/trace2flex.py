...
class Trace2FlexAdapter:
    family = "trace2flex"

    def can_parse(self, fields: list[str], path: Path) -> bool:
        required = {"timestamp", "gpu", "power_w"}
    ...
    def parse(self, path: Path) -> tuple[dict[str, Any], list[dict[str, Any]]]:
        ...
        run_id = stable_run_id(path, "trace2flex", metadata)
        _, rows = read_rows(path)
        records: list[dict[str, Any]] = []
        epochs: list[float] = []
        for index, row in enumerate(rows):
            timestamp, epoch = parse_timestamp(row.get("timestamp"), index)
            power = clean_number(row.get("power_w"))
            if power is None:
                continue
            epochs.append(epoch)
            records.append(
                {
                    ...
                    "timestamp": timestamp,
                    "time_relative_s": 0.0,
                    "gpu_id": normalize_text(row.get("gpu")),
                    "power_w": power,
                    "sm_clock_mhz": clean_number(row.get("sm_clock_mhz") or row.get("clock_sm")),
                    "gpu_util_pct": clean_number(row.get("util_gpu")),
                    "memory_util_pct": clean_number(row.get("util_mem")),
                    "memory_used_mb": clean_number(row.get("mem_used_mb")),
                    "memory_total_mb": clean_number(row.get("mem_total_mb")),
                    "temperature_c": clean_number(row.get("temp_c")),
        ...
        info = {
            ...
            "method": first_value(metadata, "method"),
            "gpu_type": first_value(metadata, "gpu_type"),
            "gpu_count": first_value(metadata, "gpu_count"),
            ...
            "grad_accum_steps": first_value(metadata, "grad_accum", "grad_accum_steps"),
            ...
            "stdout_path": resolve_source_path(path, first_value(metadata, "stdout_path", default=None)),
            ...
            "logging_method": first_value(metadata, "logging_method", default="nvidia-smi CSV"),
